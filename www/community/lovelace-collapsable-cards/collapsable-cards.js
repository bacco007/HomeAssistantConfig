class CollapsableCards extends HTMLElement {
  ...
  async renderCard() {
    ...
    card.setAttribute(
      "header",
      this._titleCard || this._config.title || "Toggle"
    );
    ...
    this.injectStyles(
      this,
      "ha-expansion-panel { --expansion-panel-content-padding: 0px }"
    );
  ...
  getEntitiesNames(card) {
    card =
      card.hasOwnProperty("tagName") &&
      this.differentConfig.includes(card.tagName)
        ? card.___config
        : card;
    ...
    if (card.hasOwnProperty("entity")) return [card.entity];

    if (card.hasOwnProperty("entities")) return card.entities;
  ...
  isCardActive() {
    return (
      this.getEntitiesNames(this._config).filter(
        (e) => this._hass.states[e].state !== "off"
      ).length > 0
    );
  }
  ...
  redrawCard(oldHass) {
    if (
      this._config.defaultOpen === "contain-toggled" &&
      this._haveEntitiesChanged(oldHass)
    )
      this.renderCard();
  }
  ...
  _haveEntitiesChanged(oldHass) {
    ...
    for (const entity of this.getEntitiesNames(this._config)) {
      if (this._hass.states[entity].state !== oldHass.states[entity].state)
        return true;
    }
