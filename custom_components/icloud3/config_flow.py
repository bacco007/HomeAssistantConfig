...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    async def async_step_review_inactive_devices(self, user_input=None, errors=None):
        ...
        if action_item == 'next_page_devices':
            return self.async_show_form(step_id='review_inactive_devices',
                    data_schema=form_review_inactive_devices(self, start_cnt=5),
                    errors=self.errors,
                    last_step=False)
        ...
        if action_item == 'inactive_to_track':
            ...
            for conf_device in Gb.conf_devices:
                ...
                if devicename not in Gb.DeviceTrackers_by_devicename:
                    self._create_device_tracker_and_sensor_entities(devicename, conf_device)
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    async def async_step_sensors(self, user_input=None, errors=None):
        ...
        if action_item == 'set_to_default_sensors':
            ...
            return self.async_show_form(step_id='sensors',
                                        data_schema=form_sensors(self, user_input=user_input),
                                        errors=self.errors)
    ...
    async def async_step_restart_ha_reload_icloud3(self, user_input=None, errors=None):
        ...
        if user_input is None or action_item is None:
            return self.async_show_form(step_id='restart_ha_reload_icloud3',
                                    data_schema=form_restart_ha_reload_icloud3(self),
                                    errors=self.errors)
...
#........................................................................................
    def _update_data_source(self, user_input):
        ...
        user_input[CONF_ICLOUD_SERVER_ENDPOINT_SUFFIX] = self.icloud_server_suffix
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    async def async_step_update_apple_acct(self, user_input=None, errors=None):
        ...
        user_input[CONF_ICLOUD_SERVER_ENDPOINT_SUFFIX] = 'cn' \
                    if user_input.get('url_suffix_china') is True else ''
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    async def async_step_reauth(self, user_input=None, errors=None,
        ...
        try:
            ...
            if ui_account_selected is not None and ui_account_selected.startswith('.'):
                action_item = 'goto_previous'
...
#----------------------------------------------------------------------
    def _check_finish_v2v3conversion_for_icloud_dname(self):
        ...
        for conf_device in Gb.conf_devices:
            ...
            if (icloud_dname != devicename
                    or devicename not in icloud_dname_by_ic3_devicename):
                continue

            icloud_dname = icloud_dname_by_ic3_devicename[devicename]
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    async def log_into_apple_account(self, user_input, called_from_step_id=None):
        ...
        icloud_server_suffix = user_input.get(CONF_ICLOUD_SERVER_ENDPOINT_SUFFIX,
                                        Gb.conf_tracking[CONF_ICLOUD_SERVER_ENDPOINT_SUFFIX])

        start_ic3.setup_icloud_server_url(icloud_server_suffix)
        ...
        if (PyiCloud
                ...
                and icloud_server_suffix == PyiCloud.icloud_server_suffix
        ...
        if icloud_server_suffix != '': event_msg += f", iCloud.com ServerSuffix-{icloud_server_suffix}"
        ...
        try:
            ...
            Gb.icloud_server_suffix   = icloud_server_suffix
            ...
            if Gb.PyiCloudLoggingInto.response_code_pwsrp_err == 503:
                ...
                if Gb.PyiCloudLoggingInto is not None:
                    ...
                    icloud_server_suffix = Gb.PyiCloudLoggingInto.icloud_server_suffix

                    if (country_code == 'CHN' and icloud_server_suffix == ''):
                        error_msg += '_cn'
                    elif (country_code != 'CHN' and icloud_server_suffix == 'cn'):
                        error_msg += '_xcn'
...
#--------------------------------------------------------------------
    def create_PyiCloudService_config_flow(self, username, password, icloud_server_suffix):
        '''
        ...
        PyiCloud = PyiCloudService( username,
                                    ...
                                    icloud_server_suffix=icloud_server_suffix,
...
#--------------------------------------------------------------------
    def _discard_changes(self, user_input):
        ...
        '''
        if user_input:
            return (user_input.get('action_item') == self.action_default_text('cancel_goto_menu'))
        else:
            return False
