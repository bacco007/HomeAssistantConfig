...
class SonarrUpcomingMediaSensor(Entity):
    ...
    def device_state_attributes(self):
        ...
        for show in self.data:
            ...
            if "network" in show["series"]:
                ...
                card_item["studio"] = ""
            if "ratings" in show["series"] and show["series"]["ratings"]["value"] > 0:
                card_item["rating"] = "\N{BLACK STAR} " + str(show["series"]["ratings"]["value"])
            else:
                card_item["rating"] = ""
            ...
            try:
                if ".jpg" in show["series"]["images"][0]["url"]:
                    card_item["fanart"] = re.sub(
                        ".jpg", "_t.jpg", show["series"]["images"][0]["url"]
                    )
                else:
                    card_item["fanart"] = ""
            except:
    ...
    def update(self):
        ...
        try:
            api = requests.get(
                "http{0}://{1}:{2}/{3}api/calendar?start={4}"
                "&end={5}".format(self.ssl, self.host, self.port, self.urlbase, start, end),
        ...
        if api.status_code == 200:
            self._state = "Online"
            if self.days == 1:
                self.data = list(filter(lambda x: x["airDate"][:-10] == str(start), api.json()))[
                    : self.max_items
                ]
            else:
