...
from .ambermodel import AmberData, PeriodSource, PeriodType
...
class AmberPricingSensor(Entity):
    ...
    def state(self):
        ...
        current_price = list(
            filter(
                lambda price: price.period_type == PeriodType.ACTUAL,
        ...
        if self.sensor_type == CONST_GENRALUSE:
            return self.calc_amber_price(
                self.amber_data.data.static_prices.e1.totalfixed_kwh_price,
                self.amber_data.data.static_prices.e1.loss_factor,
                current_price[len(current_price) - 1].wholesale_kwh_price,
            )
        if self.sensor_type == CONST_SOLARFIT:
            return self.calc_amber_price(
                self.amber_data.data.static_prices.b1.totalfixed_kwh_price,
                self.amber_data.data.static_prices.b1.loss_factor,
                current_price[len(current_price) - 1].wholesale_kwh_price,
            )
    ...
    def device_state_attributes(self):
        ...
        if self.amber_data is not None:
            ...
            for price_entry in list(
                filter(
                    lambda price: price.period_type == PeriodType.FORECAST,
                    self.amber_data.data.variable_prices_and_renewables,
                )
            ):
                entry = {}
                ...
                if self.sensor_type == CONST_GENRALUSE:
                    entry["price"] = self.calc_amber_price(
                        self.amber_data.data.static_prices.e1.totalfixed_kwh_price,
                        self.amber_data.data.static_prices.e1.loss_factor,
                        price_entry.wholesale_kwh_price,
                    )
                if self.sensor_type == CONST_SOLARFIT:
                    entry["price"] = self.calc_amber_price(
                        self.amber_data.data.static_prices.b1.totalfixed_kwh_price,
    ...
    def calc_amber_price(self, fixed_price, loss_factor, variable_price):
        return round(
            float(fixed_price) + float(loss_factor) * float(variable_price),
            2,
        )
