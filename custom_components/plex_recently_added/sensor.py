...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_SECTION_TYPES, default=["movie", "show"]): vol.All(
            cv.ensure_list, [cv.string]
        ),
        vol.Optional(
            CONF_IMG_CACHE, default="/custom-lovelace/upcoming-media-card/images/plex/"
        ): cv.string,
    }
)
...
class PlexRecentlyAddedSensor(Entity):
    def __init__(self, hass, conf, name):
        ...
        self.img = "{0}{1}{2}{3}{4}.jpg".format(self.conf_dir, {}, self._dir, {}, {})
        ...
        if self.server_name:
            self.server_ip, self.local_ip, self.port = get_server_ip(self.server_name, self.token)
        else:
        ...
        if self.change_detected:
            ...
            for media in self.data:
                ...
                if "ratingKey" in media:
                    ...
                    continue
                if "addedAt" in media:
                    card_item["airdate"] = datetime.utcfromtimestamp(media["addedAt"]).strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    )
                else:
                    continue
                ...
                if "Genre" in media:
                    card_item["genres"] = ", ".join([genre["tag"] for genre in media["Genre"]][:3])
                if media.get("rating", 0) > 0:
                    card_item["rating"] = "\N{BLACK STAR} " + str(media["rating"])
                else:
                    card_item["rating"] = ""
                if media["type"] == "movie":
                ...
                if self.dl_images:
                    ...
                    card_item["poster"] = image_url(self.url_elements, False, poster)
                    card_item["fanart"] = image_url(self.url_elements, False, fanart)
    ...
    def update(self):
        ...
        url_base = "http{0}://{1}:{2}/library/sections".format(self.ssl, self.server_ip, self.port)
        all_libraries = url_base + "/all"
        recently_added = (
            url_base + "/{0}/recentlyAdded?X-Plex-Container-" "Start=0&X-Plex-Container-Size={1}"
        )
        ...
        if libraries.status_code == 200:
            ...
            for library in sections:
                sub_sec = plex.get(
                    recently_added.format(library, self.max_items * 2), headers=headers, timeout=10
                )
                try:
                    self.api_json += sub_sec.json()["MediaContainer"]["Metadata"]
                except:
                    _LOGGER.warning(
                        'No Metadata field for "{}"'.format(
                            sub_sec.json()["MediaContainer"]["librarySectionTitle"]
                        )
                    )
                    pass
            self.api_json = sorted(self.api_json, key=lambda i: i["addedAt"], reverse=True)[
                : self.max_items
            ]
                ...
                if dir_ids != api_ids or data_ids != api_ids:
                    ...
                    for media in self.data:
                        if "type" not in media:
                            ...
                            continue
                        poster_jpg = "{}p{}.jpg".format(directory, media["ratingKey"])
                        fanart_jpg = "{}f{}.jpg".format(directory, media["ratingKey"])
                        if not os.path.isfile(fanart_jpg):
                            if image_url(self.url_elements, True, fanart):
                                image = plex.get(
                                    image_url(self.url_elements, True, fanart),
                                    headers=headers,
                                    timeout=10,
                                ).content
                        ...
                        if not os.path.isfile(poster_jpg):
                            if image_url(self.url_elements, True, poster):
                                image = plex.get(
                                    image_url(self.url_elements, True, poster),
                                    headers=headers,
                                    timeout=10,
                                ).content
...
def image_url(url_elements, cert_check, img):
    ...
    if dl_images:
        host = local
    encoded = quote(
        "http{0}://{1}:{2}{3}?X-Plex-Token={4}".format(ssl, local, port, img, token), safe=""
    )
    url = (
        "http{0}://{1}:{2}/photo/:/transcode?width=200&height=200"
        "&minSize=1&url={3}&X-Plex-Token={4}"
    ).format(ssl, host, port, encoded, token)
...
def get_server_ip(name, token):
    ...
    plex_tv = requests.get("https://plex.tv/api/servers.xml?X-Plex-Token=" + token, timeout=10)
    ...
    for server in plex_xml.findall("Server"):
        ...
        if normalize("NFKD", server_name) == normalize("NFKD", name):
            return (server.get("address"), server.get("localAddresses"), server.get("port"))
