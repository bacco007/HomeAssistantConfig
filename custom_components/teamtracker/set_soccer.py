...
async def async_set_soccer_values(new_values, event, competition_index, team_index, sensor_name) -> bool:
    """Get IN event values"""
#    new_values = {}
    teamPP = None
    oppoPP = None
...
#    _LOGGER.debug("%s: async_set_soccer_values() 3: %s", sensor_name, sensor_name)
    ...
    if teamPP and oppoPP:
        new_values["last_play"] = new_values["team_abbr"] + " " + str(teamPP) + "%, " + new_values["opponent_abbr"] + " " + str(oppoPP) + "%; "
    for detail in await async_get_value(event, "competitions", 0, "details", default=[]):
        try:
            ...
            new_values["last_play"] = new_values["last_play"] + "     " + await async_get_value(detail, "clock", "displayValue", default="{clock}")
            new_values["last_play"] = new_values["last_play"] + "  " + await async_get_value(detail, "type", "text", default="{type}")
            new_values["last_play"] = new_values["last_play"] + ": " + await async_get_value(detail, "athletesInvolved", 0, "displayName", default="{displayName}")
            if mls_team_id == new_values["team_id"]:
                new_values["last_play"] = new_values["last_play"] + " (" + new_values["team_abbr"] + ")"
            else:
                new_values["last_play"] = new_values["last_play"] + " (" + new_values["opponent_abbr"] + ")          "
        except:
