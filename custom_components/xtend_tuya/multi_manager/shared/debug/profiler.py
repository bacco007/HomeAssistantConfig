...
async def profile_async_method2(coroutine: Coroutine):
    ...
    try:
        ...
        profiler.disable()
        profiler.dump_stats(
            re.sub('[^a-z0-9._-]', '.', f'profile_async_{datetime.now()}_{coroutine}.prof', flags=re.IGNORECASE)
        )
...
async def profile_async_method(coroutine: Coroutine):
    if 'yappi' in globals() or 'yappi' in locals():
        ...
        info._save_as_PSTAT(
            re.sub('[^a-z0-9._-]', '.', f'profile_async_{datetime.now()}_{coroutine}.pstat', flags=re.IGNORECASE)
        )
    else:
